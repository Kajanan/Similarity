 ...
 *
 * <p>This evaluation does not consider negative reference cases.
 * If a positive reference case is not scored by a classifier being
 * evaluated, the method {@link #addMisses(int)} should be used to
 ...
 * with their scores and whether they were correct listed.  For this
 * example, we assume each reference positive result is scored by the
 * classifier.
 *
 * <blockquote>
 ...
 * <h3>Missing Cases</h3>
 * 
 * <p>The method {@link #addMisses(int)} provides a mechanism to
 * add counts for items that were missed by the system being
 * evaluated.  Any reference true item that is not found by
 * the system and added through {@link #addCase(boolean,double)}
 * should result in a call to <code>addMisses(1)</code>.  These
 * calls can be aggregated.
 *
 * <p>Missing cases will not arise from LingPipe's own classifiers,
 * which always return scores for all results.
 *
...
public class ScoredPrecisionRecallEvaluation {
    ...
    /**
     * Incrments the positive reference count without adding a return
    ...
    /**
     * Returns the number of cases that have been added to
     * this evaluation.
     *
    ...
    /**
     * Returns the set of recall/precision/score operating points
     * according to the scores of the cases.  Other than adding
     * scores, this method works just like {@link #prCurve(boolean)}.
     * 
     * <p>In the example in the class documentation above, the
     * scores are provided in the second column.
     *
     * @param interpolate Set to <code>true</code> if the precisions
     * are interpolated through pruning dominated points.
