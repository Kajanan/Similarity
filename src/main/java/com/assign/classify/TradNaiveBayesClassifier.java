 ...
 *          = <big><big>&Sigma;</big></big><sub>cat'</sub> p(cat'|tokens) * p(cat')</pre></blockquote>
 *
 *
 ...
 *
 * <p>A naive Bayes classifier may be compiled.  The object read back
 * in will provide a joint classifier implementing {@code
 * JointClassifier<CharSequence>}.  The compiled
 * version precomputes the log probabilities to speed run-time
 * computations.  A compiled classifier may not be trained.  In
 * order to be compiled, the tokenizer factory must be either
 * serializable or compilable.
 *
 *
...
public class TradNaiveBayesClassifier
    ...
    public JointClassification classify(CharSequence in) {
        ...
        for (String token : tokenizer) {
            ...
            if (tokenCounts == null)
                continue;
            for (int i = 0; i < mCategories.length; ++i)
                logps[i] += com.assign.util.Math.log2(probTokenByIndexArray(i,tokenCounts));
        }
        ...
        for (int i = 0; i < logps.length; ++i)
            logps[i] += com.assign.util.Math.log2(probCatByIndex(i));
    ...
    public double log2CaseProb(CharSequence input) {
        ...
        return maxJointLog2P + com.assign.util.Math.log2(sum);
    }
    ...
    static class Compiler extends AbstractExternalizable {
        ...
        public void writeExternal(ObjectOutput objOut) throws IOException {
            ...
            for (Map.Entry<String,double[]> entry : mClassifier.mTokenToCountsMap.entrySet()) {
                ...
                for (int i = 0; i < mClassifier.mCategories.length; ++i) {
                    double log2Prob = com.assign.util.Math.log2(mClassifier.probTokenByIndexArray(i,tokenCounts));
                    if (log2Prob > 0.0) {
            ...
            for (int i = 0; i < mClassifier.mCategories.length; ++i)
                objOut.writeDouble(com.assign.util.Math.log2(mClassifier.probCatByIndex(i)));
            objOut.writeDouble(mClassifier.mLengthNorm);
        }
        @Override
        public Object read(ObjectInput in) throws ClassNotFoundException, IOException {
    ...
    private static class CompiledBinaryTradNaiveBayesClassifier
        ...
        CompiledBinaryTradNaiveBayesClassifier(String[] categories,
            ...
            for (Map.Entry<String,double[]> entry : tokenToLog2ProbsInCats.entrySet()) {
                ...
                double log2ProbDiff = (log2Probs[0] - log2Probs[1]) / com.assign.util.Math.LOG2_E;
                mTokenToLog2ProbDiff.put(token,log2ProbDiff);
            }
            mLog2CatProbDiff = (log2CatProbs[0] - log2CatProbs[1]) / com.assign.util.Math.LOG2_E;
            mLengthNorm = lengthNorm;
        ...
        public JointClassification classify(CharSequence in) {
            ...
            return classification(logDiff + mLog2CatProbDiff);
        }
        ...
        JointClassification classification(double logDiff) {
            ...
            double p1 = 1.0 - p0;
            double log2P0 = com.assign.util.Math.log2(p0);
            double log2P1 = com.assign.util.Math.log2(p1);
            return (p0 > p1)
                ? new JointClassification(mCats01, new double[] { log2P0, log2P1 })
                : new JointClassification(mCats10, new double[] { log2P1, log2P0 });
        }
    }
