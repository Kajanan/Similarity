...
class CompiledHmmCharLm extends AbstractHmm {
    ...
    public CompiledHmmCharLm(ObjectInput in) 
        ...
        for (int i = 0; i < numStates; ++i)
            for (int j = 0; j < numStates; ++j)
                mTransitionLog2Probs[i][j]
                    = com.assign.util.Math.log2(mTransitionProbs[i][j]
                                                = in.readDouble());
        ...
        for (int i = 0; i < numStates; ++i)
            mStartLog2Probs[i] 
                = com.assign.util.Math.log2(mStartProbs[i] = in.readDouble());
        for (int i = 0; i < numStates; ++i)
            mEndLog2Probs[i] 
                = com.assign.util.Math.log2(mEndProbs[i] = in.readDouble());
