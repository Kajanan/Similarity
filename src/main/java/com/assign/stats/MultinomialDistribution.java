...
public class MultinomialDistribution {
    ...
    public double chiSquared(int[] sampleCounts) {
        checkNumSamples(sampleCounts);
        int totalCount = com.assign.util.Math.sum(sampleCounts);
        double sum = 0.0;
        for (int i = 0; i < sampleCounts.length; ++i)
    ...
    public static double log2MultinomialCoefficient(int[] sampleCounts) {
        ...
        long totalCount = com.assign.util.Math.sum(sampleCounts);
        double coeff = com.assign.util.Math.log2Factorial(totalCount);
        for (int i = 0; i < sampleCounts.length; ++i)
            coeff -= com.assign.util.Math.log2Factorial(sampleCounts[i]);
        return coeff;
    }
